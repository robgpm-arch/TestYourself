...
import { registrySync } from './registrySync';
 ...
 */
async function scoreRecaptchaToken(token: string, action = 'admin_access') {
...
function computeBuckets(r: any): string[] {
  const buckets = ['global'];
  if (r.state) buckets.push(`state:${r.state}`);
  if (r.state && r.district) buckets.push(`district:${r.state}-${r.district}`);
  if (r.courseId) buckets.push(`course:${r.courseId}`);
  if (r.courseId && r.subjectId) buckets.push(`subject:${r.courseId}|${r.subjectId}`);
  if (r.boardId) buckets.push(`board:${r.boardId}`);
  if (r.examId) buckets.push(`exam:${r.examId}`);
  return buckets;
}

// Maintain leaderboards when quiz results are created
export const onQuizResult = onDocumentCreated('quiz_results/{runId}', async (event) => {
  ...
  for (const b of buckets) {
    ...
    const nextScore = Math.max(Number(r.score ?? 0), Number(cur?.score ?? 0));
    batch.set(ref, {
      uid,
      displayName: pub.displayName,
      avatar: pub.avatar ?? null,
      state: pub.state ?? null,
      district: pub.district ?? null,
      score: nextScore,
      total: (cur?.total ?? 0) + 1,
      updatedAt: FieldValue.serverTimestamp(),
    }, { merge: true });
...
export const runSync = onCall(async (req) => {
  ...
  for (const col of collections) {
    if (!["mediums","boards","exams","courses","subjects","chapters","screens","themes","leaderboard_configs"].includes(col)) continue;

    try {
      const filePath = path.join(__dirname, `../registries/${col}.json`);
      const buf = fs.readFileSync(filePath, 'utf8');
      const items = JSON.parse(buf) as Array<any>;
...
export const syncMediums = onCall(async (req) => {
  if (!req.auth?.token?.admin) throw new HttpsError("permission-denied", "Admin only");

  const batch = db.batch();
  MEDIUMS_REGISTRY.forEach((medium) => {
    const ref = db.collection("mediums").doc(medium.id);
    batch.set(ref, { ...medium, updatedAt: FieldValue.serverTimestamp() }, { merge: true });
  });
  await batch.commit();
...
// Re-export registrySync and bindDevice
export { registrySync };
export { bindDevice };