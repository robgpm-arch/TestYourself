...
export type Exam = { id: string; name: string; medium?: string; board?: string; order?: number; enabled?: boolean };
...
export function listenExamsForMedium(
  ...
  const q = query(
    collection(db, "exams"),
    where("mediumId", "==", medium),
    orderBy("order")
  );
  return onSnapshot(
    q,
    snap => cb(snap.docs.map(d => ({ id: d.id, ...(d.data() as any) }))),
    err => { console.error("[exams] query error:", err); onErr?.(err); cb([]); }
  );
}