...
export function CourseSelect({
  ...
  medium: string; board?: string | ""; examId?: string | "";
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2">
        ...
        {!!value && (
          <button type="button" className="text-xs px-2 py-1 border rounded" onClick={() => onChange(null)}>
            Clear
          </button>
        )}
      </div>
      <p className="mt-1 text-xs text-slate-500">
        Lists courses for <b>{medium}</b> and {board && board !== "All Boards" ? <>board <b>{board}</b></> : <>exam <b>{examId || "—"}</b></> }.
      </p>
    </div>
  );
}