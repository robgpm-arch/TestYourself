...
const slides: Slide[] = [
  ...
  },
  {
    ...
    description: "Choose your preferred board, daily time target, and reminder schedule so the app adapts to you from day one.",
    illustration: "https://public.youware.com/users-website-assets/prod/a9adc6a9-2f03-4873-86f9-880d7d00e957/4f1acdaa1a5747b8a3d617f1b3b6df0b.png",
...
const OnboardingTutorial: React.FC<OnboardingTutorialProps> = ({ onComplete }) => {
  ...
  const scrollToSlide = useCallback((index: number) => {
    const target = slidesContainerRef.current?.querySelector<HTMLDivElement>(`[data-slide="${index}"]`);
        ...
        <AnimatePresence mode="wait" custom={currentSlide}>
          ...
          <div className="flex-1 flex flex-col items-center justify-center max-w-md mx-auto text-center">
            ...
            {currentSlide === slides.length - 1 && (
              ...
              >
                <h3 className="text-white font-semibold text-sm uppercase tracking-[0.3em] mb-3">Personalise your plan</h3>
                <div className="space-y-3">
                  <label className="block">
                    <span className="text-white/80 text-sm mb-1 block">Preferred language</span>
                    <select
                      value={onboardingPrefs.language}
                      onChange={(event) => setOnboardingPrefs(prev => ({ ...prev, language: event.target.value }))}
                  ...
                  <label className="block">
                    ...
                    <input
                      ...
                      onChange={(event) => setOnboardingPrefs(prev => ({ ...prev, board: event.target.value }))}
                  ...
                  <label className="block">
                    <span className="text-white/80 text-sm mb-1 block">Daily practice target</span>
                    <select
                      value={onboardingPrefs.dailyGoal}
                      onChange={(event) => setOnboardingPrefs(prev => ({ ...prev, dailyGoal: event.target.value }))}
                      className="w-full bg-white/15 border border-white/20 text-white rounded-xl py-2 px-3 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-300/60"
                    >
          ...
          >
            ...
            >
              <div className="onboarding-guidance-header">
                <h2><Sparkles className="w-5 h-5" />Quick Start Guide</h2>
                <button
